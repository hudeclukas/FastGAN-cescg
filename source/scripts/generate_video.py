...
def net_generate(netG, z, model_type="freeform", im_size=1024):
    ...
    if model_type == "stylegan2":
        ...
        gimg = netG(z, inject_index=8, input_is_latent=True, randomize_noise=False)[
            0
        ].cpu()
...
def batch_generate_and_save(
    ...
    for k in tqdm(range(len(zs) // batch_size)):
        gimgs = net_generate(
            netG, zs[k * batch_size : (k + 1) * batch_size], model_type, im_size=im_size
        )
        for image in gimgs:
            vutils.save_image(image.add(1).mul(0.5), folder_name + "/%d.jpg" % (num))
            num += 1
    ...
    if len(zs) % batch_size > 0:
        gimgs = net_generate(
            netG, zs[(t + 1) * batch_size :], model_type, im_size=im_size
        )
        for image in gimgs:
            vutils.save_image(image.add(1).mul(0.5), folder_name + "/%d.jpg" % (num))
            num += 1
...
def make_video_from_latents(
    ...
    for idx in range(nbr_keyframe - 1):
        main_zs += interpolate_ease_inout(
            selected_latents[idx],
            selected_latents[idx + 1],
            nbr_interpolation,
            ease_fn,
            model_type,
        )
    ...
    batch_generate_and_save(
        net,
        main_zs,
        folder_name=frames_dist_folder,
        batch_size=8,
...
if __name__ == "__main__":
    ...
    nbr_interpolation_list = [
        [fps * 60 // nbr_kf] * nbr_kf for nbr_kf in nbr_key_frames_per_minute
    ]
    nbl = []
    ...
    for idx in range(len(user_selected_noises) - 1):
        main_zs += interpolate_ease_inout(
            user_selected_noises[idx],
            user_selected_noises[idx + 1],
            nbl[idx],
            ease_fn,
            model_type,
        )
