...
def train(args):
    ...
    load_parameters(netG, ckpt["g_ema"])
    # netG.eval()
    netG.to(device)
    ...
    for iteration in tqdm(range(current_iteration, total_iterations + 1)):
        ...
        rec_loss = percept(
            F.avg_pool2d(g_image, 2, 2), F.avg_pool2d(real_image, 2, 2)
        ).sum() + 0.2 * F.mse_loss(g_image, real_image)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--batch_size", type=int, default=4, help="mini batch number of images"
    )
    parser.add_argument("--im_size", type=int, default=1024, help="image resolution")
    parser.add_argument(
        "--ckpt", type=str, default="None", help="checkpoint weight path"
    )
