...
def train(args):
    ...
    if "lmdb" in data_root:
        ...
        dataset = MultiResolutionDataset(data_root, trans, 512)
    else:
    ...
    avg_param_G = copy_Generator_parameters(netG)
    ...
    for iteration in tqdm(range(current_iteration, total_iterations + 1)):
        ...
        train_discriminator(netD, [fi.detach() for fi in fake_images], label="fake")
        optimizerD.step()
        ...
        if iteration % (save_interval * 10) == 0:
            backup_para = copy_Generator_parameters(netG)
            load_parameters(netG, avg_param_G)
            with torch.no_grad():
                vutils.save_image(
            ...
            load_parameters(netG, backup_para)
